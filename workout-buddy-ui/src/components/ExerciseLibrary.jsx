...
export default function ExerciseLibrary() {
  ...
  // Merge default + custom (customs are not persisted)
  ...
  const handleCreateExercise = () => {
    ...
    setCustomExercises(updated); // in-memory only
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="container mx-auto px-4">
        ...
        {/* Exercise Grid (each card links to the full page) */}
        {filteredExercises.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
            {filteredExercises.map((exercise) => (
              <Link
                key={exercise.id}
                to={`/exercise/${exercise.id}`}  // match your /exercises/:id route
                state={{ exercise }}              // lets custom items open without persistence
